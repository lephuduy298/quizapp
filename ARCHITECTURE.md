...
*   **Lưu trữ cấu hình / Auth đơn giản:** SharedPreferences dùng cho `UserManager`.
*   **Kết nối Mạng & API:**
...
*   **Thư viện bổ trợ:**
    *   **Accompanist Permissions:** Quản lý quyền truy cập Camera động trên Jetpack Compose.
...
Dự án áp dụng mô hình kiến trúc phân lớp sạch sẽ, tách biệt rõ ràng giữa logic giao diện, logic nghiệp vụ và dữ liệu:
...
graph TD
    ...
    UserManager -->|Auth Storage| SharedPreferences
...
3.  **`QuizViewModel`**: Quản lý toàn bộ trạng thái UI (State), luồng làm bài thi (hẹn giờ, chọn đáp án), đăng nhập/đăng ký, và kích hoạt các yêu cầu tạo đề/giải thích qua AI.
...
```text
app/src/main/
├── AndroidManifest.xml              # Tệp cấu hình ứng dụng Android (Permissions, Activities...)
├── java/com/example/
...
│   │   │   └── UserManager.kt       # Quản lý Đăng nhập/Đăng ký qua SharedPreferences
│   │   ├── remote/
